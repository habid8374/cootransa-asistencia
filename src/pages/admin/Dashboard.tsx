    ...
    <div className="space-y-5">
      ...
      <div className="flex justify-between items-center">
        ...
        <button
          onClick={cargar}
          disabled={cargando}
          className="text-xs font-semibold text-brand-700 bg-brand-50 hover:bg-brand-100 px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition disabled:opacity-60"
        >
      ...
      <div className="bg-white rounded-xl border border-gray-100 divide-y divide-gray-50">
        {cargando && datos.length === 0 ? (
          ...
          <div key={empleado.id} className="flex items-center gap-3 px-5 py-3.5">
            <div className="relative shrink-0">
              {empleado.foto_url ? (
                <img src={empleado.foto_url} className="w-10 h-10 rounded-full object-cover" alt={empleado.nombre} />
              ) : (
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-400">
                {estado === 'ausente'
                  ? 'Sin marcaciones hoy'
                  : `${estado === 'presente' ? 'Entró' : 'Salió'} a las ${fmtHora(ultimaMarca!.timestamp)}`}
                {primeraEntrada && esTardanza && empleado.hora_entrada && (
                  <span className="text-red-400 ml-1">(esperado {empleado.hora_entrada})</span>
                )}
              </p>
            </div>
