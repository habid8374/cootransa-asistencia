...
const METODOS: { key: Metodo; label: string; icon: string; desc: string }[] = [
  ...
  { key: 'tarjeta',   label: 'Tarjeta',            icon: '💳', desc: 'Crédito o débito' },
]

const NEQUI_NUM = '3001234567'  // reemplazar con el número real de COOTRANSA
...
export default function Checkout() {
  ...
  const comprar = async (e: React.FormEvent) => {
    ...
    if ((metodo === 'nequi' || metodo === 'daviplata' || metodo === 'tarjeta') && !referencia.trim()) {
      setErr('Ingresa el número de comprobante / referencia del pago.'); return
    }
    setComprando(true); setErr('')

    // Upsert pasajero por cédula
    ...
    if (pasErr || !pas) { setErr('Error al registrar pasajero.'); setComprando(false); return }
    ...
    if (tiqErr || !tiq) { setErr('Error al crear tiquete.'); setComprando(false); return }
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-brand-600 px-4 pt-4 pb-8">
        <div className="max-w-lg mx-auto">
          <Link to={`/tiquetes/viajes?origen=${viaje.ruta?.origen}&destino=${viaje.ruta?.destino}&fecha=${viaje.fecha}`}
      ...
      <div className="max-w-lg mx-auto px-4 -mt-4 pb-8">
        <form onSubmit={comprar} className="space-y-4">
            ...
            {metodo === 'tarjeta' && (
              <div className="bg-blue-50 rounded-xl p-3 space-y-2">
                <p className="text-xs font-semibold text-blue-800">
                  Próximamente: pago en línea con tarjeta vía Wompi
                </p>
                <p className="text-xs text-blue-600">Por ahora, paga en taquilla o por Nequi/Daviplata.</p>
                <input
                  value={referencia}
                  onChange={e => setReferencia(e.target.value)}
          ...
          >
            {comprando ? 'Procesando...' : `Confirmar tiquete · $${viaje.precio.toLocaleString('es-CO')}`}
          </button>
          <p className="text-center text-xs text-gray-400">Al confirmar aceptas las condiciones de transporte de COOTRANSA</p>
        </form>
      </div>
    </div>
  )
